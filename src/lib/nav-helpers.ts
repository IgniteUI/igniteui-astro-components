...
export async function fetchIgNav(lang: NavLang = 'en'): Promise<IgNavHtml> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.warn(
        `[igniteui-astro-components] Nav fetch returned ${res.status} — using static fallback.`,
      );
    }
  } catch (err: unknown) {
    console.warn(
      `[igniteui-astro-components] Could not fetch IG nav: ${(err as Error).message} — using static fallback.`,
    );
  }
