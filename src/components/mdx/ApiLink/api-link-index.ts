...
function buildCandidateNames(options: {
  ...
  for (const baseName of baseNames) {
    if (
      (!options.explicitKind || options.explicitKind === 'class') &&
      options.suffix &&
      options.classSuffix
    ) {
...
function resolveIndexedMember(
  symbol: ApiLinkIndexSymbol,
  member: string | undefined,
): { memberName: string; memberAnchor: string } | null {
...
function findIndexedSymbol(options: {
  ...
  | { name: string; symbol: ApiLinkIndexSymbol; memberName: string; memberAnchor: string }
  | { ambiguous: true; candidate: string }
  | { memberMissing: true; candidate: string }
  | null {
...
export function resolveApiLinkFromIndex(options: {
  ...
  const resolvedDocRoot =
    (indexed.symbol.p
      ? Object.values(options.ctx.apiPackages).find((pkg) => pkg.packageId === indexed.symbol.p)
          ?.docRoot
