...
function measureContentWidth(iframe: HTMLIFrameElement, body: HTMLElement): number | undefined {
    ...
    const trailing = cs ? parseFloat(cs.paddingRight || '0') + parseFloat(cs.borderRightWidth || '0') : 0;
...
function wireFitContent(iframe: HTMLIFrameElement): void {
    ...
    const measure = () => {
        ...
        } catch { /* cross-origin — rely on postMessage instead */ }
    };
    ...
    const onLoad = () => {
        ...
        iframe.contentWindow?.postMessage(
            { type: FIT_CONTENT_ENABLE_MESSAGE, width: Boolean(container?.dataset.justify) && !container?.dataset.resizable },
            '*',
        );
        measure();
        try {
        ...
        } catch { /* cross-origin — nothing to observe */ }
